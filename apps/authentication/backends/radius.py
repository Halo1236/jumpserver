...
class RADIUSBackend(object):
            ...
            try:
                ...
                if cl.lower().find(group_class_prefix) == 0:
                    ...
                    if role == "staff":
                        ...
                        logger.warning(
                            "RADIUS Attribute Class contains unknown role '%s'. "
                            "Only roles 'staff' and 'superuser' are allowed" % cl)
    ...
    def _radius_auth(self, server, username, password):
        ...
        clients = self._get_client(server)
        result = None
        for client in clients:
            packet = self._get_auth_packet(username, password, client)
            result = self._perform_radius_auth(client, packet)
            logger.debug("RADIUS auth result %s:%s" % (
