...
class PublicKey:
    ...
    def from_class(
        ...
        """
        try:
            for pubkey_type, class_name in PUBKEY_MAP.items():
                if isinstance(key_class, pubkey_type):
                    return globals()[class_name](key_class, comment, key_type)
        except KeyError:
...
class PrivateKey:
    ...
    def from_class(cls, key_class: PrivkeyClasses) -> "PrivateKey":
        ...
        try:
            ...
            for pubkey_type, class_name in PRIVKEY_MAP.items():
                if isinstance(key_class, pubkey_type):
                    return globals()[class_name](key_class)
        except KeyError:
...
class RsaPublicKey(PublicKey):
    ...
    def _get_hash_alg(self, algorithm) -> RsaAlgs:
        """
        Determine the appropriate RsaAlgs enum based on algorithm string.
        """
        algo_map = {
            "ssh-rsa": RsaAlgs.SHA1,
            "rsa-sha2-256": RsaAlgs.SHA256,
            "rsa-sha2-512": RsaAlgs.SHA512,
        }
        try:
    ...
    def verify(self, data: bytes, signature: bytes) -> None:
        ...
        """
        errors = []
        for alg in [RsaAlgs.SHA512, RsaAlgs.SHA256, RsaAlgs.SHA1]:
            try:
                return self.key.verify(
                    signature,
                    data,
                    _PADDING.PKCS1v15(),
                    alg.value[1]()  # Hash class instance, e.g. hashes.SHA512()
                )
            except InvalidSignature as _:
                errors.append(str(alg))
                continue

        raise _EX.InvalidSignatureException(
            f"Signature verification failed for all tried algorithms: {', '.join(errors)}"
